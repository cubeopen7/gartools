...
from lstm_example_reader import ptb_raw_data, ptb_producer
...
class PTBModel(object):
    def __init__(self, is_training, config, input_):
        ...
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(size, forget_bias=1.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)
        ...
        loss = tf.contrib.seq2seq.sequence_loss(logits, input_.targets, tf.ones([batch_size, num_steps], dtype=tf.float32))
        ...
        self._new_lr = tf.placeholder(tf.float32, shape=[], name="new_learning_rate")  # shape=[]是标量的意思
